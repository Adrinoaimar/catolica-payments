...
La migración `20260903000011_api_rate_limits.sql` añade buckets atómicos en PostgreSQL: 30 creaciones por usuario/minuto, 12 reconciliaciones manuales por usuario/minuto y 120 webhooks verificados por proveedor/minuto. Es un límite distribuido de aplicación; compleméntelo con un límite por IP en WAF/gateway. `20260903000012_provider_capture_race.sql` conserva en el ledger una captura TAYPI tardía como transición auditada `EXPIRED -> PAID` y elimina el RPC de expiración masiva que podía ignorar al proveedor. `20260903000013_realtime_cashier_scope.sql` limita la proyección Realtime de cajeros al día calendario de Lima.
...
2. Ejecute en orden las migraciones en `supabase/migrations/`, incluidas `20260903000008_harden_webhook_identity.sql` para instalaciones que ya tenían la función RPC anterior, `20260903000009_recoverable_payment_intents.sql` para adjuntar de forma transaccional el checkout externo a una intención pendiente, `20260903000010_realtime_delete_identity.sql` para que los eventos de eliminación sean recuperables en Realtime, `20260903000011_api_rate_limits.sql` para límites distribuidos, `20260903000012_provider_capture_race.sql` para preservar capturas tardías y `20260903000013_realtime_cashier_scope.sql` para acotar Realtime por rol y día de Lima.
3. Asigne roles mediante el mecanismo definido por las migraciones (`ADMIN` o `CASHIER`).
4. Ejecute también `20260902000002_realtime_payment_updates.sql`; crea la proyección mínima `payment_updates`, su trigger y la publicación `supabase_realtime` sin transmitir `provider_data`. La migración posterior añade `created_by`/`created_at` y aplica el límite de día para cajeros.
