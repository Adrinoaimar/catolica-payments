...
export function PaymentPage({ payment, onPaid, onNew, onSimulator, onCancelled, canCancel = false, demoMode = false }: PaymentPageProps) {
  ...
  useEffect(() => { if (current.status !== 'PENDING') return; const poll = window.setInterval(async () => { try { const response = await apiFetch(`/api/payments/${encodeURIComponent(current.reference)}`); if (!response.ok) return; const remote = await response.json() as Payment; if (remote.status !== 'PENDING') { setCurrent(remote); onPaid(remote) } } catch { /* transient network/provider failures retry on next interval */ } }, 15000); return () => window.clearInterval(poll) }, [current, onPaid])
