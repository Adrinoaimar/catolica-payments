  ...
  return <div className="page-stack cashier-page">
    ...
    <div className="cashier-layout">
      <section className="panel amount-panel"><div className="panel-topline"><div><h2>¿Cuánto deseas cobrar?</h2><p>Elige un monto rápido o ingresa uno personalizado.</p></div><span className="step-badge">01</span></div>
        <div className="quick-grid" aria-label="Montos rápidos">{quickAmounts.map((quick) => <button type="button" key={quick} className={`amount-button ${amount === quick && !custom ? 'amount-button--selected' : ''}`} onClick={() => { setAmount(quick); setCustom('') }}><span>{formatSoles(quick).replace('.00', '')}</span>{amount === quick && !custom && <i>✓</i>}</button>)}</div>
