...
export async function createPaymentRequest(amountCents: number, method: PaymentMethod, user: SessionUser): Promise<Payment> {
  ...
  // Backend remains source of truth in production. Demo fallback lets staff present full flow offline.
  try {
    const response = await fetch(method === 'CASH' ? '/api/payments/cash' : '/api/payments', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ amountCents, method }) })
    if (response.ok) {
      const remote = await response.json() as Partial<Payment> & { payment?: Partial<Payment> }
      Object.assign(payment, remote.payment ?? remote, { method })
    }
  } catch { /* use local mock when API is not running */ }
